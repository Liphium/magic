...
func (f Factory) GenerateTestFolder(path string, printFunc func(string)) (string, error) {
	...
	if data.IsDir() {
		...
		for _, f := range files {
			...
			if f.Name() == "run_test.go" {
				return "", errors.New("found a run_test.go file in your script directory: not allowed due to collision with run file")
			}
		...
		// Scan the file for test functions
		functions, err := scanTestFileForFunctions(f.ScriptDirectory(path))
		if err != nil {
			return "", err
		}
	...
	if data.IsDir() {
		...
		_, err = f.CopyToCacheWithReplacedPackage(f.ScriptDirectory(path), filepath.Join(testDir, "script.go"), "main")
		if err != nil {
			return "", fmt.Errorf("couldn't copy and replace: %s", err)
		}
	}
...
func scanTestFileForFunctions(file string) ([]string, error) {
	...
	filter1 := &FilterGoFileFunctionParameter{
		Parameters: []string{"*testing.T", "*mconfig.Plan"},
	}
	filter2 := &FilterGoFileFunctionParameter{
		Parameters: []string{"*mconfig.Plan", "*testing.T"},
	}
	results := ScanLinesSanitize(string(content), []Filter{filter1, filter2}, &CommentCleaner{})
	res1, ok1 := results[filter1]
	res2, ok2 := results[filter2]
	if !ok1 || !ok2 {
